...
CREATE TEMPORARY TABLE test_case_results (
  tst_id      CHAR(5)
, tst_descr   CHAR(255)
, start_tm    DATETIME(6)    DEFAULT CURRENT_TIMESTAMP(6)
, exec_tm     CHAR(15)
, status      CHAR(5)
, rej_dtls    TEXT
, lookup_sql  TEXT
...
AS (
	SELECT CASE WHEN row_count <> 25 THEN CONCAT('REJ: Incorrect row count|exp=25|act=', CAST(row_count AS CHAR(10)))
	            ELSE 'allgood'
	       END AS rej_dtls
...
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
	SELECT CASE WHEN row_count <> 8 THEN CONCAT('REJ: Table [countries] count where Region_ID=1 must be 8|', 'exp=8', '|act=', CAST(row_count AS CHAR(5)))
	            ELSE 'allgood'
	       END AS rej_dtls
...
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
	SELECT CASE WHEN countries_count < 5 * regions_count THEN CONCAT('REJ: Table [countries] row count must be >= 5x [regions] row count|'
	                                                               , 'exp=', CAST(regions_count * 5 AS CHAR(5)) 
	                                                               , '|act=', CAST(countries_count AS CHAR(5))
                                                                   )
	            ELSE 'allgood'
	       END AS rej_dtls
...
AS (
	SELECT 'T004' AS tst_id 
	     , CONCAT('"RS-1 Row Counts" #4 - Verify RecentRowCount() >= 5 in table [countries] where [date_last_updated] in past '
	             , '100' /*=NumberDaysLookBack*/
                 , ' days') AS tst_descr
)
, dut -- Data Under Test
AS (
	SELECT COUNT(*) AS row_count 
	FROM demo_hr.countries
	WHERE date_last_updated >= DATE_SUB(NOW(), INTERVAL 100 /*=NumberDaysLookBack*/ DAY)
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
	SELECT CASE WHEN row_count < 5 THEN CONCAT('REJ: Table [countries] count in past ' 
	                                          , '100' /*=NumberDaysLookBack*/
                                              , ' days is too low|'
											  , 'exp>=5' 
	                                          , '|act=', CAST(row_count AS CHAR(5)) )  
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT COUNT(*) FROM demo_hr.countries WHERE date_last_updated >= GETDATE() - ' 
		         , '100' /*=NumberDaysLookBack*/ ) AS lookup_sql
	FROM dut
)
-- >>>>>>>>>>>>>> Begin Boilerplate code
, hdr -- Header Row, always exists regardless whether fails exist or not 
...
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
	SELECT CASE WHEN COUNT(*) > 0 THEN CONCAT('REJ: Duplicates exist by UKey|exp=0|act=', CAST(COUNT(*) AS CHAR(10)))
	            ELSE 'allgood'
	       END AS rej_dtls
...
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
	SELECT CASE WHEN parent_id IS NULL THEN CONCAT('REJ: Orphaned region_id=', CAST(child_id AS CHAR(5)), '|exp=Exists In Tbl Regions|act=Does Not Exist')
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT * FROM demo_hr.regions WHERE region_id=', CAST(child_id AS CHAR(5)) ) AS lookup_sql
	FROM dut
)
-- >>>>>>>>>>>>>> Begin Boilerplate code
, hdr -- Header Row, always exists regardless whether fails exist or not 
...
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
	SELECT CASE WHEN child_id IS NULL THEN CONCAT('REJ: Childless (leaf node) region_id=', CAST(parent_id AS CHAR(5)), '|exp=Exists In Tbl countries|act=Does Not Exist')
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT * FROM demo_hr.countries WHERE region_id=', CAST(parent_id AS CHAR(5)) ) AS lookup_sql
	FROM dut
)
-- >>>>>>>>>>>>>> Begin Boilerplate code
, hdr -- Header Row, always exists regardless whether fails exist or not 
...
, bll -- business logic layer: apply heuristics...what constitutes a pass or a fail?
AS (
	SELECT CASE WHEN region_id = 1  AND freq_rt NOT BETWEEN 0.28 AND 0.36 THEN CONCAT('REJ-01: Frequency occurrence of region_id=1 is outside threshold|exp=0.28 thru 0.36|act=', CAST(freq_rt AS CHAR(8)) )
...
AS (
	SELECT CASE WHEN region_id IS NULL  THEN 'REJ: No nulls allowed at field region_id|exp=NoNulls|act=Null'
	            ELSE 'allgood'
	       END AS rej_dtls
	     , 'SELECT * FROM demo_hr.countries WHERE country_id=', CAST(country_id AS CHAR(15)) AS lookup_sql
	FROM demo_hr.countries
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS (
	SELECT CASE WHEN region_id < 0
                THEN CONCAT('REJ-01: Verify region_id is not negative|exp>=0|act=', CAST(region_id AS CHAR(10)) )
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT * FROM demo_hr.countries WHERE country_id=', CAST(country_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.countries
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
    SELECT CASE WHEN employee_id < 100   THEN CONCAT('REJ-01: Verify employee_id > 99|exp>99|act=', CAST(employee_id AS CHAR(10)) )
	            WHEN employee_id > 999   THEN CONCAT('REJ-02: Verify employee_id < 1000|exp<1000|act=', CAST(employee_id AS CHAR(10)) )
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT * FROM demo_hr.employees GROUP BY employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
...
AS (
	SELECT CASE WHEN region_id NOT IN(1,2,3,4)
                THEN CONCAT('REJ-01: Verify region_id in domain list (1,2,3,4) of possible values|exp=1,2,3,4|act=', CAST(region_id AS CHAR(3)) )
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT * FROM demo_hr.countries WHERE country_id=', CAST(country_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.countries
...
AS (
	SELECT CASE WHEN region_id IN(97,98,99)
                THEN CONCAT('REJ-01: Verify region_id not in domain list (97,98,99) of possible values|exp<>97,98,99|act=', CAST(region_id AS CHAR(3)) )
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT * FROM demo_hr.countries WHERE country_id=', CAST(country_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.countries
...
AS (
    SELECT CASE WHEN salary * commission_pct > 10000
                THEN CONCAT('REJ-01: Verify salary x commission_pct <= $10,000|exp<10,000|act=', CAST(salary * commission_pct AS CHAR(15)))
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT * FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN date_last_updated > NOW()        THEN CONCAT('REJ-01: Field date_last_updated cannot be in the future|exp<=', CAST(NOW() AS CHAR(20)), '|act=', CAST(date_last_updated AS CHAR(20)) )
	            ...
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT date_last_updated FROM demo_hr.countries WHERE country_id=''', country_id, '''') AS lookup_sql
	FROM demo_hr.countries
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN DATE_FORMAT(hire_date, '%H:%i:%s') <> '00:00:00' THEN CONCAT('REJ-01: Field hire_date cannot have a time part|exp=00:00:00|act=', DATE_FORMAT(hire_date, '%H:%i:%s') )
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT hire_date FROM demo_hr.employees WHERE employee_id=''', CAST(employee_id AS CHAR(10)), '''') AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN DATE_FORMAT(date_last_updated, '%H:%i:%s') = '00:00:00' THEN CONCAT('REJ-01: Field start_tm must have a time part|exp<>00:00:00|act=', DATE_FORMAT(date_last_updated, '%H:%i:%s'))
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT start_tm FROM test_case_results WHERE tst_id=''', country_id, '''') AS lookup_sql
	FROM demo_hr.countries
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS (
    SELECT CASE WHEN start_date >= end_date
	            ...
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT * FROM demo_hr.job_history WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN country_name LIKE ' %'  THEN CONCAT('REJ-02: Verify no leading space at country_name|exp=noLeadSpace|act=''', country_name, '''')
...
AS (
	SELECT CASE WHEN job_id NOT IN('ST_MAN','ST_CLERK','SH_CLERK','SA_REP','SA_MAN','PU_CLERK','PR_REP','MK_REP','MK_MAN','IT_PROG'
                ...
                THEN CONCAT('REJ-01: Verify job_id in domain list of possible values|exp=1of18|act=', JOB_ID)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , 'SELECT * FROM demo_hr.employees GROUP BY employee_id=', CAST(employee_id AS CHAR(15)) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS (
	SELECT CASE WHEN job_id IN('CEO','CFO','COO','CIO','POTUS')
                THEN CONCAT('REJ-01: Verify job_id not in domain list of excluded values|exp<>1of5|act=', job_id)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , 'SELECT * FROM demo_hr.employees GROUP BY employee_id=', CAST(employee_id AS CHAR(15)) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN email <> SUBSTRING(UPPER(CONCAT(SUBSTRING(first_name, 1, 1), last_name)), 1, 8) 
                     THEN CONCAT('REJ-01: Field email <> first char of first_name + last_name|exp=' 
	                      , SUBSTRING(UPPER(CONCAT(SUBSTRING(first_name, 1, 1), last_name)), 1, 8), '|act=', email)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT email, first_name, last_name FROM demo_hr.employees WHERE employee_id=''', CAST(employee_id AS CHAR(10)), '''') AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN LENGTH(phone_number) NOT IN(12,18)  THEN CONCAT('REJ-01: Verify phone_number length is allowed|exp=12,18|act=', CAST(LENGTH(phone_number) AS CHAR(6)))
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, phone_number, LENGTH(phone_number) FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN job_id COLLATE utf8mb4_bin <> UPPER(job_id)             THEN CONCAT('REJ-01: Verify job_id does not contain lower case characters|exp=ucase|act=', job_id)
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN employee_id REGEXP '[A-Za-z]' THEN CONCAT('REJ-01: Verify employee_id does not contain alpha characters|exp=no-alphas|act=', CAST(employee_id AS CHAR(20)))
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN first_name LIKE '%''%'  THEN CONCAT('REJ-01: Verify first_name does not contain single quote characters|exp=none|act=', first_name)
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN LOCATE(last_name, CHAR(10))  > 0 THEN CONCAT('REJ-01: Field last_name has a Line Feed (CHAR-10)|exp=none|act=at position ' 
	            ...
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, last_name FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN LOCATE(last_name, CHAR(9 using ASCII)) > 0 THEN CONCAT('REJ-01: Field last_name has a Tab (CHAR-9)|exp=none|act=at position ', CAST(LOCATE(last_name, CHAR(9 using ASCII)) AS CHAR(4)))
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, last_name FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN LOCATE(last_name, CHAR(160 using ASCII)) > 0 THEN CONCAT('REJ-01: Field last_name has a Non-Breaking-Space (CHAR-160)|exp=none|act=at position ', CAST(LOCATE(last_name, CHAR(160 using ASCII)) AS CHAR(4)))
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, last_name FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN LOCATE(last_name, CHAR(151 using ASCII)) > 0 THEN CONCAT('REJ-01: Field last_name has a Non-Breaking-Space (CHAR-151)|exp=none|act=at position ', CAST(LOCATE(last_name, CHAR(151 using ASCII)) AS CHAR(4)))
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, last_name FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN LOCATE(last_name, CHAR(11 using ASCII)) > 0  THEN CONCAT('REJ-01: Field last_name has a Vertical Tab (CHAR-11)|exp=none|act=at position ', CAST(LOCATE(last_name, CHAR(11 using ASCII)) AS CHAR(4)))
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN LOCATE(last_name, '.') > 0 THEN CONCAT('REJ-01: Field last_name has a period|exp=none|act=at position ', CAST(LOCATE(last_name, '.') AS CHAR(4)))
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN last_name REGEXP '[,/:()&#?;]' THEN CONCAT('REJ-01: Field last_name has a ",/:()&#?;" characters|exp=none|act=', last_name) 
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, last_name FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN phone_number REGEXP '[^.0123456789]' THEN CONCAT('REJ-01: Field phone_number can only have characters ".012345789"|exp=onlyAlloweChars|act=', phone_number) 
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, phone_number FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN phone_number NOT LIKE '%.%'                THEN CONCAT('REJ-01: Verify phone_number contains a ''.''|exp=contains-.|act=', phone_number)
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN zip5 REGEXP '[^0-9]' THEN CONCAT('REJ-01: Field zip5 will not convert to a number|exp=converts to number|act=', CAST(zip5 AS CHAR(5)))
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, zip5 FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS (
	SELECT CASE WHEN REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
                ...
                ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, some_date_fmt1 FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS (
	SELECT CASE WHEN REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
               ...
               ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, some_date_fmt2 FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS (
	SELECT CASE WHEN REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
               ...
               ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, some_date_fmt3 FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS (
	SELECT CASE WHEN REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
               ...
               ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, some_date_fmt4 FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS (
    ...
    SELECT CASE WHEN phone_number IS NULL THEN 'allgood'
                WHEN NOT phone_number REGEXP '^[0-9][0-9][0-9][-. ][0-9][0-9][0-9][-. ][0-9][0-9][0-9][0-9]$' THEN CONCAT('REJ-01: Field phone_number failed RegExpression check|exp=Like"###-###-####" where "-" could be " " or "." too|act=', COALESCE(phone_number, '''', '''') )
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, phone_number FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN NOT fake_ssn REGEXP '^[0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]$' THEN CONCAT('REJ-01: Field fake_ssn failed RegExpression check|exp=Like"###-##-####"|act=', fake_ssn)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, fake_ssn FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN NOT zip5 REGEXP('^[0-9][0-9][0-9][0-9][0-9]$') THEN CONCAT('REJ-01: Field zip5 failed RegExpression check|exp=Like"#####"|act=', zip5) 
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, zip5 FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN NOT zip5or9 REGEXP '^[0-9][0-9][0-9][0-9][0-9]$'
	             AND NOT zip5or9 REGEXP '^[0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9]$' THEN CONCAT('REJ-01: Field zip5or9 failed RegExpression check|exp=Like"#####" or "#####-####"|act=', zip5or9)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, zip5or9 FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN NOT zip9 REGEXP '^[0-9][0-9][0-9][0-9][0-9]-[0-9][0-9][0-9][0-9]$' THEN CONCAT('REJ-01: Field zip9 failed RegExpression check|exp=Like"#####-####"|act=', zip9)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, zip9 FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN last_name REGEXP '[^a-zA-Z ]' THEN CONCAT('REJ-01: Field last_name failed RegExpression check|exp=OnlyAlphaCharsOrSpace"|act=', last_name)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, last_name FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN zip5 REGEXP '[^0-9.]' THEN CONCAT('REJ-01: Field zip5 failed RegExpression check|exp=Like"[^0-9.]"|act=', zip5)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, zip5 FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN last_name REGEXP '^ ' OR last_name REGEXP ' $' THEN CONCAT('REJ-01: Field last_name failed check|exp=NoLeadingOrTrailingSpaces|act=', last_name)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, last_name FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN job_id REGEXP '[[:space:]]' THEN CONCAT('REJ-01: Field job_id failed RegExpression check|exp=NoSpacesTabsNewLinesNBS|act=', job_id)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, job_id FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN SUBSTRING(first_name COLLATE utf8mb4_bin, 3, 2) <> LOWER(SUBSTRING(first_name COLLATE utf8mb4_bin, 3, 2))  
	                 THEN CONCAT('REJ-01: Chars #3 and #4 for [first_name] failed check|exp=LowerCaseOnly|act=', SUBSTRING(first_name,2,2))
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, first_name FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN SUBSTRING(email COLLATE utf8mb4_bin, 3, 2) <> UPPER(SUBSTRING(email COLLATE utf8mb4_bin, 3, 2)) 
	                 THEN CONCAT('REJ-01: Field [email] failed RegExpression check|exp=UpperCaseOnly|act=', email)
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, email FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN NOT first_name COLLATE utf8mb4_bin REGEXP '^[A-Z].*'        THEN CONCAT('REJ-01: Field first_name first character not uppercase|exp=Like"^[A-Z]"|act=', first_name) 
...
, dut -- Data Under Test 
AS (
	SELECT CASE WHEN email_address IS NULL                                     THEN CONCAT('REJ-01: Field email_address is NULL', '|exp=meets biz rules|act=', email_address)
				...
				ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT employee_id, email_address FROM demo_hr.employees WHERE employee_id=', CAST(employee_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.employees
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS ( 
	SELECT CASE WHEN url NOT LIKE'http://%' 
				...
				ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT department_id, url FROM demo_hr.departments WHERE department_id=', CAST(department_id AS CHAR(15))) AS lookup_sql
	FROM demo_hr.departments
)
, bll -- Business Logic Layer: Apply heuristics...what constitutes a pass or a fail? 
AS (
...
AS (
	SELECT CASE WHEN r.region_id IS NULL            THEN CONCAT('REJ-01: Record is missing from metadata|exp=NotMissing|act=', CAST(m.region_id AS CHAR(4)), ' is missing')
	            ...
	            ELSE 'allgood'
	       END AS rej_dtls
	     , CONCAT('SELECT * FROM demo_hr.regions WHERE region_id=', CAST(m.region_id AS CHAR(15))) AS lookup_sql
	FROM      metadata   m 
...
AS (
	SELECT CONCAT('REJ-01: Mismatch Found: tbl_nm="', tbl_nm, '", job_id="', job_id, '", job_title="', job_title 
	    , '", min_salary=', CAST(min_salary AS CHAR(20)), '", max_salary=', CAST(max_salary AS CHAR(20))) AS rej_dtls
...
, bll -- business logic layer: apply heuristics...what constitutes a pass or a fail?
AS (
	SELECT CASE WHEN region_id = 1  AND freq_rt NOT BETWEEN 0.10 AND 0.50 THEN CONCAT('FAIL: Frequency occurrence of region_id=1 is FAR outside threshold|exp=0.28 thru 0.36|act=', CAST(freq_rt AS CHAR(8)))
...
AS (
	SELECT cfg.tst_id
	     , cfg.tst_descr
	     , (SELECT stus FROM hdr LIMIT 1) AS stus
	     , bll.rej_dtls
	     , bll.lookup_sql
	FROM cfg, bll
	WHERE bll.rej_dtls <> 'allgood'
...
, dut -- Data Under Test 
AS (
    SELECT CASE WHEN employee_id < 100                                            THEN CONCAT('REJ-01: Field employee_id > 99|exp>99|act=', CAST(employee_id AS CHAR(10)))
...
DROP TABLE IF EXISTS test_case_exectm;

CREATE TEMPORARY TABLE test_case_exectm (
  tst_id      CHAR(5)
, exec_tm     CHAR(15)
); 

INSERT INTO test_case_exectm
SELECT tst_id
, CONCAT(CAST(MAX(TIMESTAMPDIFF(MICROSECOND, start_tm, end_tm)) / 1000000 AS CHAR(12)), ' sec') AS exec_tm  /* 1million microseconds per second */
FROM (
...
SET SQL_SAFE_UPDATES=0;
	...
	SET exec_tm = (SELECT exec_tm
				   FROM test_case_exectm
				   WHERE tst_id = test_case_results.tst_id)
	;
...
SELECT CASE WHEN rej_dtls = '(Header)' THEN status    ELSE ' ' END AS status
     ...
     , SUBSTRING(rej_dtls, 1, NULLIF(LOCATE('|', rej_dtls) - 1, -1)) AS rej_dtls 
     , SUBSTRING(rej_dtls, LOCATE('|', rej_dtls) + 1, LENGTH(rej_dtls) - LOCATE('|', rej_dtls) - LOCATE('|', REVERSE(rej_dtls)) ) AS exp_rslt
     , REVERSE(SUBSTRING(REVERSE(rej_dtls), 1, LOCATE('|', REVERSE(rej_dtls))-1)) AS act_rslt 
     , lookup_sql
FROM rslts_ord
WHERE tst_id <> 'T999'
ORDER BY ord
