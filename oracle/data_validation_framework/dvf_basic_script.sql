...
-- T038 ------------------------------------------------------------------------------------------
    SELECT 'T038' AS tst_id
         , CASE WHEN COUNT(*) > 0 THEN 'FAIL' ELSE 'P' END AS status
         , '"RS-6 Text" #18 - Verify OnlyAllowedChars() where [phone_number] only has characters ".012345678" in table [employees]' AS tst_descr   
    FROM (
...
-- T040 ------------------------------------------------------------------------------------------
    SELECT 'T040' AS tst_id
         , CASE WHEN COUNT(*) > 0 THEN 'FAIL' ELSE 'P' END AS status
         , '"RS-6 Text" #20 - Verify IsNumeric() where [zip9] will convert to numeric in table [employees]' AS tst_descr   
    FROM (
